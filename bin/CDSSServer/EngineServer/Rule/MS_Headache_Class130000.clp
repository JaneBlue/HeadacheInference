(defrule MS_Headache_Class130000_0
(filepath ?filepath)
(Bi_Pain ?Bi_Pain)
(Discust ?Discust)
(Thrust ?Thrust)
...
(bind ?CIL130 (Leaf equals ?Slight Yes Slight))
(bind ?CIL131 (Leaf equals ?Middle Yes Middle))
(if
(or (Transform ?CIL130)  (Transform ?CIL131) )
then
(bind ?CIN120 TRUE)
else
(bind ?CIN120 NULL)
...
(bind ?CIL320 (Leaf equals ?Pressure_Pain Yes Pressure_Pain))
(if
...
(if(NotifyOrNot >= ?Threshhold 2 ?ShortData ?filepath Headache_Class130000)
then
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "慢性紧张型头痛")
(OperateFact "Headache_Diagnosis" "Chronic_Tension-type_Headache")
(FactUsed "Bi_Pain" "Discust" "Thrust" "Fire_Of_Light" "Fair_Of_Sound" "Slight" "Middle" "Worsen_By_Physicial_Activity" "Pressure_Pain")
)
)


(defrule MS_Headache_Class130000_1
(filepath ?filepath)
(Slight ?Slight)
(Middle ?Middle)
(Discust ?Discust)
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "其他慢性每日头痛")
(OperateFact "Headache_Diagnosis" "Other_Chronic_Daily_Headache")
