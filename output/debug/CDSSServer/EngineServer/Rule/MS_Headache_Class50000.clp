(defrule MS_Headache_Class50000_0
(filepath ?filepath)
(Uni_Pain ?Uni_Pain)
(Thrust ?Thrust)
(Discust ?Discust)
...
(bind ?CIL330 (Leaf equals ?Middle Yes Middle))
(bind ?CIL331 (Leaf equals ?Serious Yes Serious))
(if
...
(bind ?CIL332 (Leaf equals ?Thrust Yes Thrust))
(bind ?CIL333 (Leaf equals ?Discust Yes Discust))
(bind ?CIL334 (Leaf equals ?Fair_Of_Sound Yes Fair_Of_Sound))
(bind ?CIL335 (Leaf equals ?Fire_Of_Light Yes Fire_Of_Light))
(if
(or (Transform ?CIL332)  (Transform ?CIL333)  (Transform ?CIL334)  (Transform ?CIL335) )
then
(bind ?CIN321 TRUE)
else
(bind ?CIN321 NULL)
...
(bind ?CIN321 (AddOrNot ?CIL334 ?CIN321))
(bind ?CIN321 (AddOrNot ?CIL335 ?CIN321))
(if(eq ?CIN321 NULL)
then
(bind ?CIN321 FALSE)
)
...
(if(NotifyOrNot >= ?Threshhold 2 ?ShortData ?filepath Headache_Class50000)
then
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "ÂýÐÔÆ«Í·Í´")
(OperateFact "Headache_Diagnosis" "Chronic_Migraine")
(FactUsed "Uni_Pain" "Thrust" "Discust" "Fair_Of_Sound" "Fire_Of_Light" "Pulse_Pain" "Worsen_By_Physicial_Activity" "Middle" "Serious")
)
)


(defrule MS_Headache_Class50000_1
(filepath ?filepath)
(Uni_Pain ?Uni_Pain)
(Thrust ?Thrust)
(Discust ?Discust)
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(load (str-cat ?filepath "MS_Headache_Class130000.clp"))
