(defrule MS_Headache_Class190073_0
(filepath ?filepath)
...
(if(NotifyOrNot >= ?Threshhold 2 ?ShortData ?filepath Headache_Class190073)
then
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "很可能的丛集性头痛")
(OperateFact "Headache_Diagnosis" "Probable_Cluster_Headache")
(FactUsed "Headache_Duration_Per_Time" "Uni_Pain" "Headache_TotalNumber_Variable")
)
)


(defrule MS_Headache_Class190073_1
(filepath ?filepath)
(Headache_Duration_Per_Time ?Headache_Duration_Per_Time)
(Uni_Pain ?Uni_Pain)
(Headache_TotalNumber_Variable ?Headache_TotalNumber_Variable)
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(load (str-cat ?filepath "MS_Headache_Class190042.clp"))
(FactUsed "Headache_Duration_Per_Time" "Uni_Pain" "Headache_TotalNumber_Variable")
)
)
