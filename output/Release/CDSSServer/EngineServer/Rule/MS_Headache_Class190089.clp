(defrule MS_Headache_Class190089_0
(filepath ?filepath)
...
(if(NotifyOrNot >= ?Threshhold 1 ?ShortData ?filepath Headache_Class190089)
then
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "很可能的阵发性偏侧头痛")
(OperateFact "Headache_Diagnosis" "Probable_Paroxysmal_Hemicrania")
(FactUsed "Headache_Duration_Per_Time" "Headache_TotalNumber_Variable")
)
)


(defrule MS_Headache_Class190089_1
(filepath ?filepath)
(Headache_Duration_Per_Time ?Headache_Duration_Per_Time)
(Headache_TotalNumber_Variable ?Headache_TotalNumber_Variable)
=>
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(load (str-cat ?filepath "MS_Headache_Class190044.clp"))
(FactUsed "Headache_Duration_Per_Time" "Headache_TotalNumber_Variable")
)
)
